import os, math;from core.ErrorHandler import *;import core.Lexer
...
class Parser:
  ...
  def statement(self):
    ...
    if self.current_tok.matches(KEYWORD, 'KEMBALI'):
      res.register_advancement()
      self.advance()
    ...
    if self.current_tok.matches(KEYWORD, 'LANJUT'):
      res.register_advancement()
      self.advance()
      return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))
      
    if self.current_tok.matches(KEYWORD, 'HENTI'):
      res.register_advancement()
      self.advance()
  ...
  def expr(self):
    ...
    node = res.register(self.bin_op(self.comp_expr, ((KEYWORD, 'DAN'), (KEYWORD, 'ATAU'))))
  ...
  def comp_expr(self):
    ...
    if self.current_tok.matches(KEYWORD, 'TIDAK'):
    ...
    if res.error:
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        "Expected int, float, '+', '-', '(', '[', or FUNCTION"
      ))
  ...
  def atom(self):
    ...
    elif tok.matches(KEYWORD, 'JIKA'):
      if_expr = res.register(self.if_expr())
      if res.error: return res
      return res.success(if_expr)

    elif tok.matches(KEYWORD, 'LOOP'):
      for_expr = res.register(self.for_expr())
      if res.error: return res
      return res.success(for_expr)

    elif tok.matches(KEYWORD, 'LOOP2'):
    ...
    return res.failure(InvalidSyntaxError(
      ...
      "Expected FUNCTION [LOOP, LOOP2] and Other"
    ))
  ...
  def if_expr_b(self):
    return self.if_expr_cases('MUNGKIN')
    
  def if_expr_c(self):
    res = ParseResult()
    else_case = None

    if self.current_tok.matches(KEYWORD, 'KECUALI'):
      res.register_advancement()
      self.advance()
      ...
      if self.current_tok.type == NEWLINE:
        ...
        if self.current_tok.matches(KEYWORD, 'TUTUP'):
          res.register_advancement()
          self.advance()
        else:
          return res.failure(InvalidSyntaxError(
  ...
  def if_expr_b_or_c(self):
    ...
    if self.current_tok.matches(KEYWORD, 'MUNGKIN'):
  ...
  def if_expr_cases(self, case_keyword):
    ...
    if not self.current_tok.matches(KEYWORD, 'TERUS'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'TERUS'"
      ))
  ...
  def for_expr(self):
    ...
    if not self.current_tok.matches(KEYWORD, 'LOOP'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'LOOP'"
      ))
    ...
    if not self.current_tok.matches(KEYWORD, 'UNTUK'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'UNTUK'"
      ))
    ...
    if self.current_tok.type == NEWLINE:
      ...
      if not self.current_tok.matches(KEYWORD, 'TUTUP'):
        return res.failure(InvalidSyntaxError(
          self.current_tok.pos_start, self.current_tok.pos_end,
          f"Expected 'TUTUP'"
        ))
  ...
  def while_expr(self):
    ...
    if not self.current_tok.matches(KEYWORD, 'LOOP2'):
      return res.failure(InvalidSyntaxError(
        self.current_tok.pos_start, self.current_tok.pos_end,
        f"Expected 'LOOP2'"
      ))
...
class Interpreter:
  ...
  def visit_BinOpNode(self, node, context):
    ...
    elif node.op_tok.matches(KEYWORD, 'DAN'):
      result, error = left.anded_by(right)
    elif node.op_tok.matches(KEYWORD, 'ATAU'):
...
global_symbol_table.set("KATAKAN", BuiltInFunction.print)
...
def Tokenrun(fn, text):
  lexer = core.Lexer.Lexer(fn, text)
